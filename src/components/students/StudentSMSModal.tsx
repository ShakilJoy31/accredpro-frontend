// components/students/StudentSMSModal.tsx
"use client";
...
interface StudentSMSModalProps {
    isOpen: boolean;
    studentId: number;
    phoneNumber: string;
    studentName: string;
...
const HARDCODED_SMS_CONFIG = {
    id: 1,
    appName: "FIT INFOTECH",
    apiKey: "C400102369708eca4d1604.85945999",
    type: "unicode",
    senderId: "FIT INFO",
    message: "Welcome to FIT INFOTECH! Thank you for enrolling in our course. Stay connected for updates.",
    status: true
};

const StudentSMSModal: React.FC<StudentSMSModalProps> = ({
    isOpen,
    studentId,
    phoneNumber,
    studentName,
    ...
    const [sendStatus, setSendStatus] = useState<{
        success?: number;
        failed?: number;
        results?: Array<{ success: boolean; phoneNumber: string; error?: string; messageUsed?: string }>;
        errors?: Array<{
            phoneNumber: string;
            error: string;
            response?: unknown;
    ...
    const smsConfigs = [HARDCODED_SMS_CONFIG];
    const isLoadingConfigs = false;
    const selectedConfigId = 1;
    const selectedConfig = HARDCODED_SMS_CONFIG;
    const configMessage = selectedConfig?.message || "";
    ...
    const handleSendSMS = async () => {
        ...
        try {
            ...
            const result = await sendSMS({
                ...
                messages: useConfigMessage ? undefined : [messageToSend]
            }).unwrap();
            ...
            if (result.success) {
                if (result.data.results && result.data.results.length > 0) {
                    ...
                    if (sentResult.success) {
                        ...
                        toast.success(`To: ${studentName} (${phoneNumber}) - ${messageType} message sent successfully`);
                    } else {
                        toast.error(`To: ${studentName} - ${sentResult.error || 'Failed to send'}`);
                    }
                }
                ...
                if (onSuccess && result.data.failed === 0) {
                    setTimeout(() => {
                        onSuccess();
                    }, 1500);
                }
                ...
                <div className={`p-6 border-b transition-colors duration-300 ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'
                    ...
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-lg transition-colors duration-300 ${theme === 'dark' ? 'bg-orange-500/20' : 'bg-orange-100'
                                }`}>
                ...
                <div className="p-6 space-y-6 max-h-[50vh] overflow-y-auto">
                    ...
                    <div className={`p-3 rounded-lg ${theme === 'dark' ? 'bg-gray-700/50' : 'bg-gray-50'
                        ...
                        <p className={`text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'
                            ...
                            SMS Configuration: <span className="font-bold">{HARDCODED_SMS_CONFIG.appName}</span>
                        </p>
                        <p className={`text-xs mt-1 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'
                            }`}>
                            Sender ID: {HARDCODED_SMS_CONFIG.senderId} • Type: {HARDCODED_SMS_CONFIG.type}
                        </p>
                    </div>

                    {/* Student Info */}
                    <div className={`p-3 rounded-lg ${theme === 'dark' ? 'bg-blue-500/10 border border-blue-500/30' : 'bg-blue-50 border border-blue-200'}`}>
                        <p className={`text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>
                            Sending to: <span className="font-semibold">{studentName}</span>
                        </p>
                        <p className={`text-xs mt-1 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                            Student ID will be included for reference
                        </p>
                    </div>
                        ...
                        <div className={`p-3 rounded-lg ${
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <p className={`text-sm font-medium ${
                                    ...
                                    {sendStatus.failed === 0
                                        ? `SMS sent successfully to ${studentName}`
                                        : `Failed to send SMS to ${studentName}`
                                    }
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        {isScheduling ? (
                            ...
                            <motion.button
                                ...
                                disabled={isSending || !selectedConfigId || !phoneNumber.trim() || (!message.trim() && !configMessage.trim())}
...
export default StudentSMSModal;