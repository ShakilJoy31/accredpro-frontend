// components/certificate/EditCertificate.tsx
"use client";
...
import { useGetCertificateByIdQuery, useUpdateCertificateMutation } from "@/redux/api/sms-configurations/certificateApi";
...
export default function EditCertificate() {
  ...
  const { data: certificateData, isLoading, refetch } = useGetCertificateByIdQuery(certificateId, {
    skip: !certificateId,
  });
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          <div className="flex items-center gap-2">
            ...
            <span>Certificate updated successfully!</span>
          </div>,
          { duration: 3000 }
        );
        refetch();
      ...
      toast.error(
        <div className="flex items-center gap-2">
          ...
          <span>{error?.data?.message || "Failed to update certificate"}</span>
        </div>,
        { duration: 4000 }
      );
    } finally {
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <select
                  className="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                  {...register("status")}
                >
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden">
          <div className="p-6 border-b border-gray-200 dark:border-gray-800 bg-gradient-to-r from-gray-50 to-white dark:from-gray-800 dark:to-gray-900">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Files (Optional)</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400">Upload certificate file or logo</p>
              </div>
            </div>
          </div>
          ...
          >
            {isSubmitting ? (
              <>
                ...
                <span>Updating Certificate...</span>
              </>
            ) : (
              <>
