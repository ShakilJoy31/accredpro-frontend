...
const SellTransactionModal: React.FC<SellTransactionModalProps> = ({
  isOpen,
  transaction,
  onClose
}) => {
    ...
    >
      ...
      >
          ...
          {transaction.retailerName && (
            <div className={`p-4 rounded-xl ${theme === 'dark' ? 'bg-gray-800/50' : 'bg-gray-50'}`}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Company</span>
              </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {transaction.payment > 0 && (
              <div className={`p-4 rounded-xl text-center ${theme === 'dark' ? 'bg-green-500/10' : 'bg-green-50'}`}>
                ...
                <p className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>Received</p>
              </div>
            )}
          </div>

          {/* Balance and Discount */}
          <div className={`grid ${transaction.discount > 0 ? 'grid-cols-2' : 'grid-cols-1'} gap-4`}>
            <div className={`p-4 rounded-xl ${transaction.balance > 0 ? 'bg-orange-500/10' : 'bg-green-500/10'}`}>
              <p className={`text-sm text-center mb-1 ${transaction.balance > 0 ? 'text-orange-400' : 'text-green-400'}`}>
                Remaining Balance
              </p>
              <p className={`text-2xl text-center font-bold ${transaction.balance > 0 ? 'text-orange-500' : 'text-green-500'}`}>
                {formatCurrency(transaction.balance)}
              </p>
            </div>
            {transaction.discount > 0 && (
              <div className={`p-4 rounded-xl ${theme === 'dark' ? 'bg-purple-500/10' : 'bg-purple-50'}`}>
                <p className={`text-sm mb-1 text-center text-purple-400`}>Discount Applied</p>
        ...
        <div className={`p-6 border-t ${theme === 'dark' ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
          <button
            onClick={onClose}
            className={`w-full cursor-pointer py-3 rounded-xl transition-all duration-300 ${theme === 'dark'
